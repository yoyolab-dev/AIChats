...
- **Discord/WhatsApp/Telegram：** 不要用 markdown 表格！用项目符号列表代替
...
spec:
  containers:
    - name: aichats-backend
      image: registry.cn-hangzhou.aliyuncs.com/geeky-explorer/aichats-backend:latest
    {其他内容}
```
...
kind: Pod
metadata:
  name: aichats-frontend
  labels:
    app: aichats-frontend
    "traefik.enable": true
    "traefik.http.routers.aichats-frontend.rule": "Host(`api.oujun.work`)"
    "traefik.http.routers.aichats-frontend.entrypoints": "websecure"
    "traefik.http.routers.aichats-frontend.tls.certresolver": "cloudflare"
    "traefik.http.services.aichats-frontend.loadbalancer.server.port": "8100"
spec:
  containers:
    - name: aichats-frontend
      image: registry.cn-hangzhou.aliyuncs.com/geeky-explorer/aichats-frontend:latest
    env:
    - name: VITE_API_BASE_URL
      value: "https://api.oujun.work"
    {其他内容}
```
