...
describe('FriendshipService', () => {
  ...
  describe('sendRequest', () => {
    ...
    it('should throw error if sending request to self', async () => {
      await expect(new FriendshipService().sendRequest('user-1', 'user-1')).rejects.toThrow(
        'Cannot send friend request to yourself',
      );
    });

    it('should throw error if already friends', async () => {
      _prisma.friendship.findFirst.mockResolvedValue({
        id: 'f-123',
        userId: 'user-1',
        friendId: 'user-2',
        status: 'ACCEPTED',
      });
      await expect(new FriendshipService().sendRequest('user-1', 'user-2')).rejects.toThrow(
        'Already friends',
      );
    });
  ...
  describe('handleRequest', () => {
    it('should accept friend request', async () => {
      const friendship = {
        ...
        friend: { id: 'user-1', username: 'alice' },
      };
      ...
      expect(_prisma.friendship.update).toHaveBeenCalledWith({
        where: { id: 'f-123' },
    ...
    it('should throw error if friendship not found', async () => {
      ...
      await expect(
        new FriendshipService().handleRequest('nonexistent', 'user-1', 'accept'),
    ...
    it('should throw error if not authorized', async () => {
      ...
      _prisma.friendship.findUnique.mockResolvedValue(friendship);
      await expect(
        new FriendshipService().handleRequest('f-123', 'other-user', 'accept'),
      ).rejects.toThrow('Not authorized to handle this request');
    });
  });
  ...
  describe('getFriends', () => {
    it('should return list of accepted friends', async () => {
      ...
      expect(_prisma.friendship.findMany).toHaveBeenCalledWith({
  ...
  describe('getIncomingRequests', () => {
    it('should return incoming friend requests', async () => {
      ...
      expect(_prisma.friendship.findMany).toHaveBeenCalledWith({
        where: { friendId: 'user-1', status: 'PENDING' },
        include: expect.any(Object),
        orderBy: { createdAt: 'desc' },
      });
    });
  ...
  describe('removeFriendship', () => {
    it('should remove friendship between two users', async () => {
      ...
      expect(_prisma.friendship.deleteMany).toHaveBeenCalledWith({
        where: {
          OR: [
            { userId: 'user-1', friendId: 'friend-1' },
            { userId: 'friend-1', friendId: 'user-1' },
          ],
        },
      });
    });
